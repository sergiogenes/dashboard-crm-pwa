...
- **Configuración de PWA y Service Worker:**
  ...
  - **Prevención de Condiciones de Carrera (Race Conditions) en Sincronización**: Al descargar actualizaciones del CRM (Inbound Pull), los resolvedores y las funciones auxiliares de sincronización (como `syncActivitiesForLead` o `syncDealsForLead`) nunca deben sobreescribir datos locales de MongoDB si el documento tiene cambios locales pendientes de sincronizar (`crmSynced: false` o `synced: false`). Las modificaciones locales del usuario en el dashboard tienen prioridad absoluta sobre los datos temporales del CRM.
